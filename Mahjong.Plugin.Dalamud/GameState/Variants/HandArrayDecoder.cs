...
internal static class HandArrayDecoder
{
    ...
    /// <summary>
    /// Index in the visible, gap-compressed hand row for the preferred addon
    /// slot. After a call, slots 10..12 can be empty while the drawn tile stays
    /// in slot 13; counting only populated slots preserves the visual index.
    /// </summary>
    public static int FindRenderedIndex(ReadOnlySpan<int> rawSlots, int textureBase, int targetTileId)
    {
        int addonSlot = FindAddonSlot(rawSlots, textureBase, targetTileId);
        if (addonSlot < 0)
            return -1;

        int renderedIndex = 0;
        for (int i = 0; i < addonSlot; i++)
        {
            if (DecodeTileId(rawSlots[i], textureBase, out _) >= 0)
                renderedIndex++;
        }
    ...
    /// <summary>
    /// Visible hand-row indexes for a multiset of tile IDs, matching each
    /// requested tile to a distinct addon slot (a Pon consumes two physical
    /// copies of the same kind). Requested tiles without a remaining match are
    /// skipped. The result is sorted ascending.
    /// </summary>
    public static List<int> FindRenderedIndexes(
        ReadOnlySpan<int> rawSlots, int textureBase, ReadOnlySpan<int> targetTileIds)
    {
        var result = new List<int>(targetTileIds.Length);
        Span<bool> used = rawSlots.Length <= 32 ? stackalloc bool[32] : new bool[rawSlots.Length];
        ...
        foreach (int targetTileId in targetTileIds)
        {
            ...
            for (int i = 0; i < rawSlots.Length; i++)
            {
                if (!used[i] && DecodeTileId(rawSlots[i], textureBase, out _) == targetTileId)
                {
                    addonSlot = i;
                    break;
                }
