...
public sealed class AddonEmjReader : IDisposable
{
    ...
    internal AddonEmjAtkValueProbe? AtkValueProbe { get; set; }
        ...
        IPluginLog log, string pluginConfigDir, string layoutsDir, IFindingsLog? findings)
    {
        try
        {
            ...
            log.Info(
                $"[MjDebug] Loaded {variants.Count} layout profile(s) from {layoutsDir}: " +
        ...
        {
            log.Error($"[MjDebug] Layout profile load failed at {layoutsDir}: {ex.Message}");
    ...
    /// <summary>
    /// Visible hand-row index for the preferred raw addon slot. Unlike a search
    /// through the compressed snapshot hand, this keeps slot 13 preferred after
    /// a call and skips empty raw slots before it.
    /// </summary>
    public unsafe int FindRenderedHandIndexOfTile(Mahjong.Core.Tile target)
    {
        if (ActiveLayout is null)
            return -1;
        if (!addon.TryGet(out var unit, out _))
        ...
        for (int i = 0; i < len; i++)
            raw[i] = *(int*)(basePtr + ActiveLayout.Offsets.HandArrayStart + i * 4);
        int eff = Variants.HandArrayDecoder.ResolveTextureBase(raw, ActiveLayout.TileTextureBase, out _);
        return Variants.HandArrayDecoder.FindRenderedIndex(raw, eff, target.Id);
    }

    /// <summary>
    /// Visible hand-row indexes for the tiles consumed by a recommended call,
    /// mapping duplicates (a Pon uses two copies of one kind) to distinct
    /// physical slots. Tiles that cannot be located are omitted.
    /// </summary>
    public unsafe List<int> FindRenderedHandIndexesOfTiles(IReadOnlyList<Mahjong.Core.Tile> targets)
    {
        if (targets.Count == 0 || ActiveLayout is null)
            return [];
        if (!addon.TryGet(out var unit, out _))
            return [];
        if (!unit->IsVisible)
        ...
        Span<int> ids = stackalloc int[targets.Count];
        for (int i = 0; i < targets.Count; i++)
            ids[i] = targets[i].Id;
        return Variants.HandArrayDecoder.FindRenderedIndexes(raw, eff, ids);
    }
    ...
    public unsafe StateSnapshot? TryBuildSnapshot()
    {
        ...
        // Fill opponent public information from the visible table whenever the
        // memory layout does not expose the actual river tiles.  Discard counts
        // from the addon remain authoritative; the visual reader supplies the
        // tile sequence and sideways-riichi marker only when it can decode them.
        if (snap is not null)
        {
            // Reconstruct rivers/opponent melds from the visible table, then
            // merge PreRefresh code-8 tails so a mid-hand load keeps the visual
            // prefix instead of discarding an incomplete event stream.
            snap = VisualPublicStateReader.Apply(unit, snap, log);
            if (AtkValueProbe is not null)
                snap = AtkValueProbe.ApplyPublicState(snap);
            AgentEmjPublicStateProbe.Observe(snap, pluginConfigDir, log);
        }
