...
{
    private const string Primary = "/mjdebug";
    private const string HelpText = "Open the Doman Mahjong Solver window. Type /mjdebug help for the full command list.";
    ...
    private void OnCommand(string command, string args)
    {
        ...
        switch (sub)
        {
            ...
            case "on":
                ...
                chatGui.Print("[MjDebug] Automation armed.");
                break;

            case "off":
                plugin.ConfigService.Update(c => c with { AutomationArmed = false });
                chatGui.Print("[MjDebug] Automation disarmed.");
                break;
            ...
            case "discardhook":
            {
                ...
                chatGui.Print(
                    $"[MjDebug] DiscardCapture  strategy={capture.StrategyName}  " +
            ...
            default:
                chatGui.PrintError($"[MjDebug] Unknown subcommand: '{sub}'. Type /mjdebug help for the command list.");
                break;
        }
    }
    ...
    {
        chatGui.Print("Doman Mahjong Solver Debug — /mjdebug commands");
        chatGui.Print("  /mjdebug — open the plugin window");
        chatGui.Print("  /mjdebug on | off — arm / disarm automation");
        chatGui.Print("  /mjdebug help — show this help");
        ...
        chatGui.Print("  /mjdebug debug — toggle the developer console");

        chatGui.Print("Manual override:");
        chatGui.Print("  /mjdebug autodiscard — run the policy once and discard");
        chatGui.Print("  /mjdebug testdiscard <0..13> — dispatch a discard at a slot");
        chatGui.Print("  /mjdebug pass <0..5> — send a raw call-option index");

        chatGui.Print("Capture for bug reports:");
        chatGui.Print("  /mjdebug log <on|off> — record clicks to emj-events.log");
        chatGui.Print("  /mjdebug capture <label> — capture a single click");
        chatGui.Print("  /mjdebug snap <label> — dump addon + agent memory to a file");
        chatGui.Print("  /mjdebug autosnap <on|off> — auto-snap on state changes");

        chatGui.Print("Reverse-engineering dumps:");
        chatGui.Print("  /mjdebug variant dump — layout dump for new client variants");
        chatGui.Print("  /mjdebug walknodes — dump the AtkUld node tree");
        chatGui.Print("  /mjdebug findtiles — scan addon/agent memory for tile-encoded values");
        chatGui.Print("  /mjdebug poolslots — diff visible discard-pool slots for tile fields");
        chatGui.Print("  /mjdebug poolicons — decode discard-pool slot icon ids");
        chatGui.Print("  /mjdebug addons [filter] — list loaded addons");
        chatGui.Print("  /mjdebug dumpmem [offset] [length] — hex dump of addon memory");
        chatGui.Print("  /mjdebug atkvalues — dump the Emj AtkValues array");
        chatGui.Print("  /mjdebug hexdump [agent] [start] [end] — safe hex dump of addon or AgentEmj");
        chatGui.Print("  /mjdebug agentprobe — capture safe AgentEmj header and readable pointer targets");
        chatGui.Print("  /mjdebug discardhook — print DiscardCapture health");
    }
    ...
    internal void HandleAutoDiscard()
    {
        framework.RunOnFrameworkThread(() =>
        {
            ...
            if (snap is null)
            {
                chatGui.PrintError("[MjDebug] no snapshot — not in a match.");
                return;
            }
            ...
            {
                chatGui.PrintError(
                    $"[MjDebug] hand has {snap.Hand.Count} tiles — not a discard state. Wait for your turn.");
                return;
            }
            ...
            {
                chatGui.PrintError(
                    $"[MjDebug] policy returned {choice.Kind} — autodiscard only handles Discard. {choice.Reasoning}");
                return;
            }
            ...
            if (slot < 0)
            {
                chatGui.PrintError($"[MjDebug] tile {tile} not found in hand — internal error.");
                return;
            }

            var delay = Actions.HumanTiming.RandomDelay();
            chatGui.Print(
                $"[MjDebug] auto-discarding {tile} (slot {slot}) in {delay.TotalMilliseconds:F0}ms. {choice.Reasoning}");

            _ = framework.RunOnTick(() =>
            {
                var result = plugin.Dispatcher.DispatchDiscard(slot);
                chatGui.Print($"[MjDebug] dispatch result: {result}");
            }, delay);
        });
    }
    ...
    internal void HandlePass(string arg)
    {
        ...
        {
            chatGui.PrintError("[MjDebug] Usage: /mjdebug pass <0..5>  (0=leftmost, higher=rightward; rightmost = pass)");
            return;
        }
        framework.RunOnFrameworkThread(() =>
        {
            var result = plugin.Dispatcher.DispatchCallOption(opt);
            chatGui.Print($"[MjDebug] pass opt={opt} → {result}");
        });
    }
    ...
    internal void HandleTestDiscard(string arg)
    {
        ...
        {
            chatGui.PrintError("[MjDebug] Usage: /mjdebug testdiscard <0..13>");
            return;
        }
        ...
        {
            var result = plugin.Dispatcher.DispatchDiscard(slot);
            chatGui.Print($"[MjDebug] testdiscard slot={slot} result={result}");
        });
    }
    ...
    internal void HandleCapture(string arg)
    {
        ...
        if (string.IsNullOrEmpty(label))
        {
            ...
            if (pending != null)
            {
                ...
                chatGui.Print($"[MjDebug] capture disarmed (was: {pending}).");
            }
            else
            {
                chatGui.Print(
                    $"[MjDebug] Usage: /mjdebug capture <label>. Run again with no label to disarm. " +
        ...
        foreach (var c in label)
        {
            ...
            {
                chatGui.PrintError(
                    $"[MjDebug] capture label must be [a-zA-Z0-9_-] only — got '{label}'.");
                return;
            }
        }
        ...
        {
            chatGui.PrintError(
                "[MjDebug] auto-play is ON — its dispatches would race your manual click. " +
                "Run `/mjdebug off` first, then re-arm capture.");
            return;
        }

        plugin.EventLogger.ArmCapture(label);
        chatGui.Print(
            $"[MjDebug] capture armed: '{label}'. Click the action in-game once. " +
    ...
    private void HandleVariant(string arg)
    {
        ...
        switch (sub)
        {
            case "dump":
                ...
                break;
            case "":
                chatGui.Print("[MjDebug] Usage: /mjdebug variant dump");
                break;
            default:
                chatGui.PrintError(
                    $"[MjDebug] Unknown variant subcommand: '{sub}'. Known: dump.");
                break;
        }
    }
    ...
    internal unsafe void DumpVariant()
    {
        framework.RunOnFrameworkThread(() =>
        {
            ...
            {
                chatGui.PrintError(
                    "[MjDebug] Mahjong addon not found — seat at a table first, then retry.");
                return;
            }
            ...
            {
                var agent = agentModule->GetAgentByInternalId(AgentId.Emj);
                if (agent == null)
                {
                    sb.AppendLine("  (AgentEmj not found — GetAgentByInternalId(AgentId.Emj) returned null)");
                }
                else
                {
    ...
    internal unsafe void HandleSnap(string arg)
    {
        ...
        {
            chatGui.PrintError(
                "[MjDebug] Usage: /mjdebug snap <label>  (label: [a-zA-Z0-9_-] only)");
            return;
        }
        foreach (var c in label)
        {
            ...
            {
                chatGui.PrintError(
                    $"[MjDebug] snap label must be [a-zA-Z0-9_-] only — got '{label}'.");
                return;
            }
        }
        ...
        framework.RunOnFrameworkThread(() =>
        {
            ...
            if (path != null)
                chatGui.Print($"[MjDebug] snap '{label}' → {path}");
        });
    }
    ...
    internal void HandleAutoSnap(string arg)
    {
        ...
        switch (v)
        {
            case "on":
                if (autoSnapOn)
                {
                    chatGui.Print("[MjDebug] autosnap already ON.");
                    return;
                }
                autoSnapOn = true;
                autoSnapCounter = 0;
                ...
                plugin.AddonReader.ObservationChanged += OnAutoSnapObservation;
                chatGui.Print(
                    $"[MjDebug] autosnap ON. Hash-deduped, min gap {AutoSnapMinGapMs}ms, cap {AutoSnapMaxCount}. " +
            ...
            case "off":
                if (!autoSnapOn)
                {
                    chatGui.Print("[MjDebug] autosnap already OFF.");
                    return;
                }
                autoSnapOn = false;
                plugin.AddonReader.ObservationChanged -= OnAutoSnapObservation;
                chatGui.Print($"[MjDebug] autosnap OFF. Wrote {autoSnapCounter} file(s).");
                break;

            case "":
                chatGui.Print(
                    $"[MjDebug] autosnap is {(autoSnapOn ? "ON" : "OFF")} " +
                    $"(wrote {autoSnapCounter}/{AutoSnapMaxCount}).");
                break;

            default:
                chatGui.PrintError("[MjDebug] Usage: /mjdebug autosnap <on|off>");
                break;
        }
    }
    ...
    private unsafe void OnAutoSnapObservation(AddonEmjObservation obs)
    {
        ...
        if (autoSnapCounter >= AutoSnapMaxCount)
        {
            ...
            plugin.AddonReader.ObservationChanged -= OnAutoSnapObservation;
            chatGui.Print(
                $"[MjDebug] autosnap hit cap ({AutoSnapMaxCount}) — auto-disarmed. " +
        ...
        {
            Plugin.Log.Warning($"[MjDebug] autosnap hash failed: {ex.GetType().Name}: {ex.Message}");
            return;
        }
        ...
        {
            Plugin.Log.Warning($"[MjDebug] autosnap write '{label}' failed: {ex.GetType().Name}: {ex.Message}");
        }
    }
    ...
    private unsafe string? WriteSnapFile(string label, bool verbose)
    {
        ...
        {
            if (verbose)
                chatGui.PrintError("[MjDebug] Emj addon not found — open a table first.");
            return null;
        }
        nint addonAddr = (nint)unit;
        ...
        if (agentModule != null)
        {
            agent = agentModule->GetAgentByInternalId(AgentId.Emj);
            if (agent != null)
            {
                byte* agentPtr = (byte*)agent;
    ...
    internal unsafe void HandlePoolIcons()
    {
        framework.RunOnFrameworkThread(() =>
        {
            ...
            System.IO.File.WriteAllText(path, sb.ToString());
            chatGui.Print($"[MjDebug] poolicons → {path}  ({totalDecoded} decoded)");
        });
    }
    ...
    internal unsafe void HandlePoolSlots()
    {
        framework.RunOnFrameworkThread(() =>
        {
            ...
            System.IO.File.WriteAllText(path, sb.ToString());
            chatGui.Print($"[MjDebug] poolslots → {path}  ({slotsByType.Sum(kvp => kvp.Value.Count)} visible slots)");
        });
    }


    internal unsafe void HandleAgentProbe()
    {
        framework.RunOnFrameworkThread(() =>
        {
            ...
            if (module == null)
            {
                chatGui.PrintError("[MjDebug] AgentModule unavailable.");
                return;
            }

            var agent = module->GetAgentByInternalId(AgentId.Emj);
            if (agent == null)
            {
                chatGui.PrintError("[MjDebug] AgentEmj unavailable.");
                return;
            }

            var sb = new System.Text.StringBuilder();
            sb.AppendLine($"# AgentEmj safe probe  base=0x{(nint)agent:X}  utc={DateTime.UtcNow:o}");
            sb.AppendLine();

            if (SafeMemoryReader.TryRead((nint)agent, 0x1000, out var header))
            {
                sb.AppendLine("## Agent header +0x0000..+0x1000");
                for (int off = 0; off < header.Length; off += 16)
                {
                    int count = Math.Min(16, header.Length - off);
                    sb.Append($"+0x{off:X4}: ");
                    for (int i = 0; i < count; i++)
                        sb.Append($"{header[off + i]:X2} ");
                    sb.AppendLine();
                }
            }
            else
            {
                sb.AppendLine("## Agent header unreadable");
            }

            var pointers = SafeMemoryReader.FindReadablePointers((nint)agent, 0x200, 0x200);
            sb.AppendLine();
            sb.AppendLine($"## Readable pointer candidates ({pointers.Count})");
            int index = 0;
            foreach (var (offset, pointer) in pointers.Take(32))
            {
                sb.AppendLine($"### [{index++:D2}] agent+0x{offset:X3} -> 0x{pointer:X}");
                if (!SafeMemoryReader.TryRead(pointer, 0x200, out var target))
                {
                    sb.AppendLine("  unreadable");
                    continue;
                }
                for (int off = 0; off < target.Length; off += 16)
                {
                    sb.Append($"  +0x{off:X4}: ");
                    int count = Math.Min(16, target.Length - off);
                    for (int i = 0; i < count; i++)
                        sb.Append($"{target[off + i]:X2} ");
                    sb.AppendLine();
                }
            }
            ...
            var dir = pluginInterface.GetPluginConfigDirectory();
            System.IO.Directory.CreateDirectory(dir);
            var path = System.IO.Path.Combine(dir, $"agent-emj-safe-probe-{DateTime.UtcNow:yyyyMMdd-HHmmss}.txt");
            System.IO.File.WriteAllText(path, sb.ToString());
            chatGui.Print($"[MjDebug] safe AgentEmj probe → {path}");
        });
    }
    ...
    internal unsafe void HandleHexDump(string args)
    {
        framework.RunOnFrameworkThread(() =>
        {
            ...
            if (agentMode)
            {
                var agentModule = AgentModule.Instance();
                if (agentModule == null)
                { chatGui.PrintError("[MjDebug] AgentModule unavailable."); return; }
                var agent = agentModule->GetAgentByInternalId(AgentId.Emj);
                if (agent == null)
                { chatGui.PrintError("[MjDebug] AgentEmj unavailable."); return; }
                basePtr = (byte*)agent;
            ...
            if (!SafeMemoryReader.TryRead((nint)basePtr + start, end - start, out var safeBytes))
            {
                chatGui.PrintError("[MjDebug] requested range is not fully readable; capture cancelled.");
                return;
            }
            for (int rel = 0; rel < safeBytes.Length; rel += 16)
            {
                int count = Math.Min(16, safeBytes.Length - rel);
                sb.Append($"+0x{start + rel:X4}: ");
                for (int i = 0; i < count; i++)
                    sb.Append($"{safeBytes[rel + i]:X2} ");
                sb.AppendLine();
            }
            ...
            System.IO.File.WriteAllText(path, sb.ToString());
            chatGui.Print($"[MjDebug] hexdump → {path}  ({(end - start) / 16} rows)");
        });
    }
    ...
    internal unsafe void HandleFindTiles()
    {
        framework.RunOnFrameworkThread(() =>
        {
            ...
            System.IO.File.WriteAllText(path, sb.ToString());
            chatGui.Print($"[MjDebug] findtiles → {path}  ({totalMatches} matches)");
        });
    }
    ...
    internal unsafe void HandleWalkNodes()
    {
        framework.RunOnFrameworkThread(() =>
        {
            ...
            System.IO.File.WriteAllText(path, sb.ToString());
            chatGui.Print($"[MjDebug] walknodes → {path}  ({mgr.NodeListCount} nodes)");
        });
    }
    ...
    internal void HandleLog(string arg)
    {
        ...
        switch (v)
        {
            case "on":
                ...
                chatGui.Print($"[MjDebug] event logger ON. Writing to {plugin.EventLogger.LogPath}");
                break;
            case "off":
                plugin.EventLogger.Enabled = false;
                plugin.EventLogger.CloseLog();
                chatGui.Print("[MjDebug] event logger OFF.");
                break;
            case "":
                chatGui.Print(
                    $"[MjDebug] event logger is {(plugin.EventLogger.Enabled ? "ON" : "OFF")}. " +
                    $"Path: {plugin.EventLogger.LogPath}");
                break;
            default:
                chatGui.PrintError("[MjDebug] Usage: /mjdebug log <on|off>");
                break;
        }
    }
    ...
    internal unsafe void DumpAtkValues()
    {
        if (!addon.TryGet(out var unit, out _))
        {
            chatGui.PrintError("[MjDebug] Emj addon not found.");
            return;
        }
        ...
        {
            chatGui.PrintError("[MjDebug] AtkValues is null or empty.");
            return;
        }
        ...
        chatGui.Print($"[MjDebug] wrote {count} AtkValues to {path}");
    }
    ...
    internal unsafe void DumpMemory(string args)
    {
        ...
        {
            chatGui.PrintError($"[MjDebug] bad offset '{parts[0]}'. Use hex, optional 0x prefix.");
            return;
        }
        if (parts.Length >= 2 && !string.IsNullOrEmpty(parts[1]) && !TryParseHex(parts[1], out length))
        {
            chatGui.PrintError($"[MjDebug] bad length '{parts[1]}'. Use hex, optional 0x prefix.");
            return;
        }
        ...
        chatGui.Print($"[MjDebug] wrote 0x{length:X} bytes @ +0x{offset:X} to {path}");
    }
    ...
    internal unsafe void DumpAddons(string filter)
    {
        ...
        if (stage == null)
        {
            chatGui.PrintError("[MjDebug] AtkStage not available (not in game?)");
            return;
        }
        ...
        for (int i = 0; i < unitManagers.Count; i++)
        {
            ...
            chatGui.Print(
                $"[MjDebug] {name,-24} @ 0x{(nint)unit:X}  vis={unit->IsVisible}");
            count++;
        }
        chatGui.Print($"[MjDebug] {count} addon(s) {(string.IsNullOrEmpty(filterLower) ? "total" : $"matching \"{filter}\"")}.");
    }
