...
public static class PluginServices
{
        ...
        Configuration configuration)
    {
        ...
        services.AddSingleton<IConfigMigrator<Configuration>, ConfigMigratorV1ToV2>();
        services.AddSingleton<IConfigMigrator<Configuration>, ConfigMigratorV2ToV3>();
        services.AddSingleton<IConfigMigrator<Configuration>, ConfigMigratorV3ToV4>();
        services.AddSingleton<IConfigMigrator<Configuration>, ConfigMigratorV4ToV5>();
        services.AddSingleton<IConfigMigrator<Configuration>, ConfigMigratorV5ToV6>();
    }
    ...
    private static void RegisterPolicies(IServiceCollection services)
    {
        ...
        services.AddSingleton<SelectablePolicy>(sp =>
            new SelectablePolicy(
                sp.GetRequiredService<EfficiencyPolicy>(),
                sp.GetRequiredService<IConfigService<Configuration>>(),
                sp.GetRequiredService<global::Dalamud.Plugin.Services.IPluginLog>(),
                sp.GetRequiredService<IDalamudPluginInterface>().AssemblyLocation.DirectoryName ?? string.Empty));

        services.AddSingleton<IPolicy>(sp => sp.GetRequiredService<SelectablePolicy>());
    }
}
