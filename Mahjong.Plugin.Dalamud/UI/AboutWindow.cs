...
public sealed class AboutWindow : Window, IDisposable
{
    ...
    {
        "個人開発者として、空き時間にFFXIV向け自動化プラグインを制作しています。",
        "このプラグインが役立った場合は、他のプレイヤーへの共有がプロジェクト支援になります。",
        "不具合報告、機能要望、その他のフィードバックはGitHub Discussionsで受け付けています。",
        "ご利用ありがとうございます。",
    };
    ...
    public AboutWindow(IPluginLog log, IDalamudPluginInterface pluginInterface, ITextureProvider textureProvider)
        : base("ドマ式麻雀ソルバー デバッグ版: このプラグインについて###domanmahjong-debug-about")
    {
    ...
    private void DrawDetailsTable()
    {
        ...
        DrawTextRow("作者", Author);
        DrawLinkRow("GitHub", RepoUrl);
        DrawLinkRow("不具合を報告", IssuesUrl);
        DrawLinkRow("ディスカッション", DiscussionsUrl);
        DrawLinkRow("セキュリティ問題を報告", SecurityUrl);
    ...
    private void DrawLinkRow(string label, string url)
    {
        ...
        ImGui.BeginTooltip();
        ImGui.TextUnformatted("クリックで開く・右クリックでコピー");
        ImGui.EndTooltip();
