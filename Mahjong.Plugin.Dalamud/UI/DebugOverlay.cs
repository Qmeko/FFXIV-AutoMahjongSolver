...
public sealed class DebugOverlay : Window, IDisposable
{
    ...
    public DebugOverlay(
        ...
        : base("ドマ式麻雀 · 開発者ツール###domanmahjong-debug")
    {
    ...
    public override void Draw()
    {
        ...
        {
            DrawTab("状態", statusTab.Draw);
            DrawTab("アドオン", addonTab.Draw);
            DrawTab("操作", actionsTab.Draw);
            DrawTab("不具合報告", bugReportTab.Draw);
            DrawTab("解析", reProbesTab.Draw);
            DrawTab("診断", diagnosticsTab.Draw);
            ImGui.EndTabBar();
        }
    ...
    private void DrawHeaderCard()
    {
        ...
        using (Theme.BeginCard("debug-header"))
        {
            ...
            ImGui.TextWrapped("メイン画面で利用規約に関する注意事項へ同意するまで、自動操作は無効です。");
            ImGui.PopStyleColor();
            if (ImGui.Button("同意して有効化"))
