...
public sealed class ActionStateMachine
{
    ...
    /// <summary>True only between the one-time Riichi selection and its selected discard.</summary>
    ...
    /// <summary>
    /// Clears all transient dispatch and per-hand state. Used when automation is
    /// explicitly disarmed so no queued/retry context can survive into a later
    /// manual or automated decision.
    /// </summary>
    public void Reset()
    {
        inFlight = false;
        dispatchStartedAt = default;
        lastActionAt = default;
        lastContext = null;
