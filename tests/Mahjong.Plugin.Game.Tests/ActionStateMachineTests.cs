...
public class ActionStateMachineTests
{
    ...
    [Fact]
    public void RiichiConfirm_can_be_cleared_immediately_after_the_selected_discard()
    {
        var fsm = NewFsm();
        fsm.LatchRiichiConfirm(Tile.FromId(26));
    ...
    [Fact]
    public void Clearing_dispatch_context_does_not_duplicate_or_drop_the_riichi_latch()
    {
        var fsm = NewFsm();
        var tile = Tile.FromId(26);
        fsm.LatchRiichiConfirm(tile);
        fsm.ClearContext();
