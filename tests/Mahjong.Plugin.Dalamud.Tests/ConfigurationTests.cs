...
public class ConfigurationTests
{
    ...
    public void Defaults_match_documented_values()
    {
        ...
        Assert.Equal(3500, c.HumanizedDelayMs);
        Assert.True(c.ShowInGameHighlight);
        Assert.False(c.ShowSuggestionDetails);
        Assert.True(c.EnableGameLogging);
        Assert.Equal(AiProvider.BundledMortal, c.AiProvider);
        Assert.Equal(AkochanInferenceProfile.Realtime, c.AkochanInferenceProfile);
        Assert.Equal(5000, c.ExternalAiTimeoutMs);
        Assert.Equal(120000, c.ExternalAiStartupTimeoutMs);
        Assert.False(c.MortalOnline);
        Assert.True(c.ExternalAiFallbackToBuiltIn);
    }
