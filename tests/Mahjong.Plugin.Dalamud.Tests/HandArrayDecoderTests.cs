...
public class HandArrayDecoderTests
{
    ...
    [Fact]
    public void Rendered_index_post_pon_prefers_slot_13_and_skips_gap()
    {
        int rendered = HandArrayDecoder.FindRenderedIndex(
            PostPonGapRaw(),
            EmjTextureBase,
            targetTileId: 32);

        // Raw slot 13 is the preferred drawn tile. Ten populated slots precede
        // it, while the empty raw slots 10..12 do not occupy visual positions.
        Assert.Equal(10, rendered);
    }
